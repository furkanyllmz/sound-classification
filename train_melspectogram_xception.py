...
# =================== AYARLAR ===================
MODEL_NAME = 'Xception'  # Komuttan da alabilir
if len(sys.argv) > 1:
    MODEL_NAME = sys.argv[1]

TARGET_SIZE = (224, 224)  # Base modeller için güvenli hedef boyut
RESULTS_DIR = f'results/melspectrogram_{MODEL_NAME.lower()}'
...
# =================== MELSPECTOGRAM ÇIKARIM ===================
def extract_melspectrogram(file_path, n_mels=128, max_pad_len=128):
    """Melspectogram özelliklerini ses dosyasından çıkarır"""
    try:
        audio, sample_rate = librosa.load(file_path, res_type='kaiser_fast', duration=4.0)
        
        # Melspectogram hesapla
        ...
        print(f"Melspectogram çıkarma hatası {file_path}: {e}")
        return None

def load_dataset(dataset_path='dataset'):
    """Tüm ses dosyalarını yükler ve melspectogram özelliklerini çıkarır"""
    features, labels = [], []
    ...
    for label in classes:
        ...
        for filename in os.listdir(class_path):
            if filename.endswith('.wav'):
                ...
                melspec = extract_melspectrogram(file_path)
...
X_rgb = np.repeat(X[..., np.newaxis], 3, -1)  # (128, 128, 3) melspectogram için
...
# 6) Örnek Melspectogram görselleştirmesi
print("\nÖrnek melspectogram görselleştirmeleri oluşturuluyor...")
n_show = min(6, len(X_test))
fig, axes = plt.subplots(2, 3, figsize=(15, 10))
axes = axes.ravel()
for i in range(n_show):
    axes[i].imshow(X_test[i, :, :, 0], aspect='auto', origin='lower', cmap='magma')
    predicted_class = class_names[y_pred_classes[i]]
    true_class = class_names[y_true_classes[i]]
    color = 'green' if predicted_class == true_class else 'red'
    axes[i].set_title(f'True: {true_class}\nPred: {predicted_class}', color=color, fontweight='bold')
    axes[i].set_xlabel('Time'); axes[i].set_ylabel('Mel Frequency Bins')
plt.suptitle(f'{MODEL_NAME} Transfer Learning: Sample Melspectogram Predictions', fontsize=16, fontweight='bold')
plt.tight_layout()
...
print("\n" + "="*80)
print(f"{MODEL_NAME} Melspectogram Transfer Learning Model Eğitimi Tamamlandı!")
print("="*80)
...
print("\nKaydedilen dosyalar:")
print(f"  • results.json - Ana sonuçlar ve metrikler")
print(f"  • history.csv - Epoch bazında training history")
print(f"  • epoch_metrics.csv - Detaylı epoch metrikleri")
print(f"  • training_history.png - Training/validation grafikler")
print(f"  • accuracy_by_epoch.png - Accuracy grafiği")
print(f"  • confusion_matrix.png - Confusion matrix")
print(f"  • sample_predictions.png - Örnek melspectogram tahminleri")
print("="*80)
